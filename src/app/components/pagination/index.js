...
export default function Pagination({ base, currentPage, pageNumber, total, textPosition }) {
  ...
  const totalText = <div style={{ marginTop: textPosition === 'bottom' ? 14 : 0, marginBottom: textPosition === 'top' ? 14 : 0 }}>Showing {(currentPage - 1) * ITEMS_PER_PAGE + 1}-{pageTotal < total ? pageTotal : total } of {total}</div>
