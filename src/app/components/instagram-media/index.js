...
export default function InstagramMedia({ media, expandGalleries, isBR, withoutLink, fullQuality, isMain, hasPoster, isListing }) {
  const host = useHost();
  const i18n = useI18n();

  const mediaElement = media.file_type === 'video' || media.file.includes('.mp4') ? <>{hasPoster && isListing ? <img src={FILE_DOMAIN_500 + media.file.replace('.mp4', '-thumb.png')} alt={isBR ? media.description_pt : media.description} loading="lazy" /> : <video src={FILE_DOMAIN + media.file + (!hasPoster ? '#t=0.1' : '')} controls poster={hasPoster ? (fullQuality ? FILE_DOMAIN : FILE_DOMAIN_500) + media.file.replace('.mp4', '-thumb.png') : ''} />}</> : <img src={FILE_DOMAIN + media.file} srcSet={ `${(fullQuality ? FILE_DOMAIN : FILE_DOMAIN_500) + media.file} 500w` } alt={isBR ? media.description_pt : media.description} loading="lazy" />;
