...
export default function InstagramMedia({ media, expandGalleries, isBR, withoutLink, fullQuality, isMain, isListing }) {
  ...
  return <div key={ media.file } className={ 'instagram_media_gallery_item' + (isMain && media.type === 'instagram-story' && media.mode === 'portrait' ? ' ' + 'instagram_media_portrait' : '') + (media.gallery && media.gallery.length && ! expandGalleries ? ' ' + 'instagram_media_is_gallery' : '' ) } itemScope itemType={media.file.includes('.mp4') && !isListing ? 'http://schema.org/VideoObject' : 'http://schema.org/ImageObject' }>
    {withoutLink ? <div style={{ cursor: media.file.includes('.mp4') ? '' : 'zoom-in' }}>{mediaElement}</div> : <Link href={link} style={{ display: 'block' }} prefetch={false}>
      ...
      <div className={ 'instagram_media_play_button' }><img src="/images/play.svg" alt={i18n('Play Button')} /></div>}
    </Link>}
    ...
    {!media.is_gallery && media.locations && media.location_data && media.location_data[0] && <div style={{marginTop: 4}} className={'instagram_media_location'}>
