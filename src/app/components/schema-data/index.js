...
export default function SchemaData({ media, isWebStories = false, isExpand = false }) {
  const host = useHost();
  ...
  const content = <>
    ...
    <span itemProp="uploadDate" content={media.date ? media.date.replace(' ', 'T') + '+03:00' : media.cityData.end + 'T12:00:00+03:00'}/>
    ...
    {media.file && media.file.includes('.mp4') && <>
      <span itemProp="duration" content={serialize({ seconds: Math.ceil(media.duration) })}/>
      <span itemProp="thumbnailUrl" content={FILE_DOMAIN + media.file.replace('.mp4', '-thumb.png')}/>
    </>}
  </>

  const optionalContent = <>
    {media.location_data && media.location_data.length > 0 && <span itemProp="contentLocation" content={media.location_data.map(l => (isBR && l.name_pt ? l.name_pt : l.name) + (l.alternative_names && l.alternative_names.length ? ' (' + l.alternative_names.join(', ') + ')' : '')).join(', ')}/>}
    {keywords && keywords.length > 0 && <span itemProp="keywords" content={'#' + keywords.filter(l => l).join(' #')}/>}
  </>;
