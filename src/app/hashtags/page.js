...
export default async function MapPage() {
  ...
  const allHashtags = isBR ? allHashtagsData.hashtags_pt : allHashtagsData.hashtags;
  ...
  return <div>
    ...
    <div className="container">
      ...
      <div>
        ...
        <HashtagCloud theHashtags={hashtags.filter(h => !h.is_place).map((h) => ({ text: '#' + (isBR && h.name_pt ? h.name_pt : h.name), value: h.total }))} isBR={isBR} />
      </div>

      <div>
        <h3>{i18n('Random Hashtags')}</h3>
        <HashtagCloud theHashtags={shuffle(allHashtags).slice(0, 100).map(c => ({ text: '#' + c, value: 5 }))} isBR={isBR} shuffleText={i18n('Shuffle')} isRandom />
      </div>
    </div>
  </div>
}
