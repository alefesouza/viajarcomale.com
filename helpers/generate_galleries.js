...
for (item of items.filter(
  (i) => i.hashtags.includes('seoul') && !i.hashtags.includes('saopaulo')
)) {
  const gallery =
    galleries.find((g) =>
      g.find((i) => {
  ...
  item.gallery = gallery
    .filter((g) => g.image !== item.image)
    .map((g, i) => ({
      ...g,
      file:
        '/photos/south-korea/seoul/' +
        item.id +
        '-' +
        (i + 1) +
        (g.file.includes('.jpg') ? '.jpg' : '.mp4'),
    }));
  ...
  images.forEach((image, i) => {
    ...
    fs.rename(
      image,
      'south-korea/seoul/' + id + (image.includes('.jpg') ? '.jpg' : '.mp4'),
      function (err) {
