...
const theHighlights = {};
stories.reverse().forEach(async (item) => {
  if (!fs.existsSync('./to_send/' + item.country + '/' + item.city)) {
    fs.mkdirSync('./to_send/' + item.country + '/' + item.city, {recursive: true})
  }

  if (!fs.existsSync('./to_send/500/' + item.country + '/' + item.city)) {
    fs.mkdirSync('./to_send/500/' + item.country + '/' + item.city, {recursive: true})
  }

  if (!fs.existsSync('./to_send/portrait/' + item.country + '/' + item.city)) {
    fs.mkdirSync('./to_send/portrait/' + item.country + '/' + item.city, {recursive: true})
  }

  if (!fs.existsSync('./to_send/landscape/' + item.country + '/' + item.city)) {
    fs.mkdirSync('./to_send/landscape/' + item.country + '/' + item.city, {recursive: true})
  }

  if (!fs.existsSync('./to_send/square/' + item.country + '/' + item.city)) {
    fs.mkdirSync('./to_send/square/' + item.country + '/' + item.city, {recursive: true})
  }
  ...
  const fileToSend = './to_send/' + item.country + '/' + item.city + '/' + item.id + (item.file.includes('.mp4') || !item.file.includes('.') ? '.mp4' : '.jpg');
  ...
  sharp(fileToSend.replace('.mp4', '-thumb.png'))
    ...
    .resize(500)
    .jpeg({ mozjpeg: true })
    .toFile(fileToSend.replace('.mp4', '-thumb.png').replace('/to_send/', '/to_send/500/'), (err, info) => { console.log(err) });

  imageMagick(fileToSend.replace('.mp4', '-thumb.png'))
    .crop(item.width, item.width * 0.75, 0, (item.height / 2) - (item.width * 0.75 / 2))
      .write(fileToSend.replace('.mp4', '-thumb.png').replace('/to_send/', '/to_send/landscape/'), function (err) {
        if (err) {
          console.log(err);
          return;
        }
  ...
  imageMagick(fileToSend.replace('.mp4', '-thumb.png'))
    .crop(item.width, item.width * 1.25, 0, (item.height / 2) - (item.width * 1.25 / 2))
      .write(fileToSend.replace('.mp4', '-thumb.png').replace('/to_send/', '/to_send/portrait/'), function (err) {
        if (err) {
          console.log(err);
          return;
        }
  ...
  imageMagick(fileToSend.replace('.mp4', '-thumb.png'))
    .crop(item.width, item.width, 0, (item.height - item.width) / 2)
      .write(fileToSend.replace('.mp4', '-thumb.png').replace('/to_send/', '/to_send/square/'), function (err) {
        if (err) {
          console.log(err);
          return;
        }
  ...
  item.file = '/stories/' + item.country + '/' + item.city + '/' + item.id + (item.file.includes('.mp4') || !item.file.includes('.') ? '.mp4' : '.jpg');
  item.hashtags = [...(item?.hashtags?.split(' ') || []), ...(item?.newHashtags?.split(` / `) || [])].join(' ')
...
fs.writeFileSync('./the-locations.json', JSON.stringify(stories.filter(c => c.location_data).flatMap(c => c.location_data), null, 4))
